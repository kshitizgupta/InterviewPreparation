 ...
 * contiguous sub arrays it can make, keep storing the sum for all of them so we can ultimately
 * get the max sum possible. But in this case the complexity will be O(n^2) as for first element
 * we will be forming n-1 contiguous sub arrays, for 2nd we will have n-2, so on and so forth
 * n+(n-1)+(n-2)+.....+1 = n^2
 *
